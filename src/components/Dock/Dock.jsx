...
function DockItem({ children, className = "", onClick, mouseX, spring, distance, magnification, baseItemSize }) {
  ...
  return (
    <motion.div ref={ref} style={{ width: size, height: size, position: "relative", zIndex: 2 }}
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div initial={{ opacity: 0, y: 0 }} animate={{ opacity: 1, y: -10 }} exit={{ opacity: 0, y: 0 }}
          transition={{ duration: 0.2 }} className={`dock-label ${className}`} role="tooltip" style={{ x: "-50%", pointerEvents: "none" }}
        >{children}</motion.div>
      )}
    </AnimatePresence>
  );
  ...
  return (
    <motion.div className="dock-outer" style={{ overflow: "visible" }}>
      <motion.div
        ...
        className={`dock-panel ${className}`} style={{ height: panelHeight, overflow: "visible" }} role="toolbar"
      >
